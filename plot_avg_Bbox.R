for(para in c("DNA_bending_stiffness", "DNA_denaturation", "Duplex_disrupt_energy", "Duplex_free_energy" ,"Protein_induced_deformability_Bp" ,"Stabilizing_energy_of_Z_DNA_AS", "Stabilizing_energy_of_Z_DNA_SA", "Stacking_energy", "Bruckner" ,"Packer")){
  ...
  for(i_chr in c(1:22,"X","Y","M")){
    if(!sum(list.files()==sprintf("%s_chr%s_Bbox.txt", para,i_chr))){next}
    dpp <- rbind(dpp, read.table(sprintf("/Users/uemurakohei/Desktop/ncRNA/tRNA/avgDPP_a_b_box/%s/%s_chr%s_Bbox.txt", para,para,i_chr), header=T))
  }
  ...
  pdf(sprintf("%s_avg_B_box.pdf", para),width=3.4, height=4)
  ...
  lines(x[(length(x)-11):(length(x)-11-11)], rep(y_lim[1], length((length(x)-11):(length(x)-11-11))), col="red",lwd=2,lty="dashed")
